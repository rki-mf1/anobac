################################################################################
# AnoBac | Plot Kleborate v3.2.0 summary
#
...
kleborate <- read_csv(opt$i, show_col_types = FALSE) %>%
    rename_with(~ gsub("^klebsiella_pneumo_complex__", "", .x)) %>%
    rename_with(~ gsub("^klebsiella__", "", .x)) %>%
    rename_with(~ gsub("^mlst__", "", .x)) %>%
    rename_with(~ gsub("^kaptive__", "", .x)) %>%
    rename_with(~ gsub("^virulence_score__", "", .x)) %>%
    rename_with(~ gsub("^resistance_score__", "", .x))
